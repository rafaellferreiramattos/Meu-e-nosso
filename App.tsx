...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const initApp = async () => {
            ...
            // 1. Get All Users (Async now)
            const allUsers = await authService.getAllUsers();
            setUsers(allUsers);
            
            // 2. Get Current User (Async now)
    ...
    // Load Groups logic remains mostly same but depends on currentUser
    useEffect(() => {
        if (currentUser) {
    ...
    // ... (All Derived State and Handlers remain same, except login/logout) ...
    ...
    const groupTransactions = useMemo(() => {
        ...
        return transactions.filter(t => t.groupId === selectedGroupId).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    const { notifications: systemNotifications, markAsRead: markSystemAsRead, clearAll: clearSystemAll } = useNotifications({
        currentUser, selectedGroupId, selectedGroupMembers: selectedGroup ? selectedGroup.members : [], debts, goals, contributions, revenues: userRevenues, invitations, transactionsTotal: groupTransactions.reduce((sum, t) => sum + t.amount, 0), users
    });
    ...
    const clearAll = () => {
        clearSystemAll();
        if (currentUser) setCustomNotifications(prev => prev.filter(n => n.userId !== currentUser.id));
    };
    ...
    const notifyGroupMembers = (type: 'expense' | 'goal' | 'contribution', title: string, message: string, actionLink: string) => {
        ...
        if (newNotifs.length > 0) setCustomNotifications(prev => [...prev, ...newNotifs]);
    };
    ...
    // ... (Other handlers unchanged, just ensuring no async issues) ...
    ...
    const handleAddExpense = useCallback((newTransaction: Omit<Transaction, 'id'>) => {
        setTransactions(prev => [...prev, { ...newTransaction, id: `t${Date.now()}` }]);
        setIsExpenseModalOpen(false);
        if (currentUser) {
            notifyGroupMembers('expense', 'Nova Despesa Adicionada', `${currentUser.name} adicionou "${newTransaction.description}" (R$ ${newTransaction.amount.toFixed(0)}).`, 'Despesas');
        }
    }, [currentUser, selectedGroup]);

    const handleAddRevenue = useCallback((newRevenue: Omit<Revenue, 'id' | 'userId'>) => {
        if (!currentUser) return;
        setRevenues(prev => [...prev, { ...newRevenue, id: `r${Date.now()}`, userId: currentUser.id }]);
        setIsRevenueModalOpen(false);
    }, [currentUser]);
    const handleToggleRevenueReceived = useCallback((id: string) => setRevenues(prev => prev.map(r => r.id === id ? { ...r, received: !r.received } : r)), []);
    const handleDeleteRevenue = useCallback((id: string) => setRevenues(prev => prev.filter(r => r.id !== id)), []);
    const handleUpdateTransaction = useCallback((updatedTransaction: Transaction) => { setTransactions(prev => prev.map(t => t.id === updatedTransaction.id ? updatedTransaction : t)); setViewingTransaction(null); }, []);
    const handleDeleteTransaction = useCallback((transactionId: string) => { setTransactions(prev => prev.filter(t => t.id !== transactionId)); setViewingTransaction(null); }, []);
    
    const handleAddGroup = useCallback((newGroupData: Omit<Group, 'id' | 'members'>) => {
        const newGroup: Group = { ...newGroupData, id: `g${Date.now()}`, members: [] };
    ...
    const handleUpdateGroupMembers = useCallback((groupId: string, newMemberIds: string[]) => {
        setGroups(prevGroups => prevGroups.map(g => g.id === groupId ? { ...g, memberIds: newMemberIds } : g));
    ...
    const handleSelectGroup = (groupId: string) => { setSelectedGroupId(groupId); setActiveView('Dashboard'); setIsMobileMenuOpen(false); };
    
    const handleAddGoal = useCallback((newGoalData: Omit<Goal, 'id' | 'groupId'>) => {
        if (!selectedGroupId) return;
        const newGoal: Goal = { ...newGoalData, id: `goal${Date.now()}`, groupId: selectedGroupId };
        setGoals(prev => [...prev, newGoal]);
        setIsAddGoalModalOpen(false);
        if(currentUser) notifyGroupMembers('goal', 'Nova Meta Criada', `${currentUser.name} criou a meta "${newGoal.name}" (Alvo: R$ ${newGoal.targetAmount}).`, 'Metas');
    ...
    const handleAddContribution = useCallback((newContributionData: Omit<Contribution, 'id'>) => {
        ...
        const newContribution: Contribution = { ...newContributionData, id: `c${Date.now()}` };
        ...
        if (totalBefore < goal.targetAmount && totalAfter >= goal.targetAmount) {
            ...
            notifyGroupMembers('goal', 'Meta Atingida! 🎉', `A meta "${goal.name}" foi concluída com sucesso!`, 'Metas');
        } else {
            const contributor = users.find(u => u.id === newContribution.userId);
            if(contributor) notifyGroupMembers('contribution', 'Nova Contribuição', `${contributor.name} adicionou R$ ${newContribution.amount.toFixed(0)} para "${goal.name}".`, 'Metas');
        }
    ...
    const handleOpenAddContributionModal = (goal: Goal) => { setSelectedGoalForContribution(goal); setIsAddContributionModalOpen(true); };
    ...
    const handleSendInvitation = useCallback((toFriendId: string): 'success' | 'not_found' | 'already_friend' | 'pending' | 'self' => {
        ...
        const newInvitation: Invitation = { id: `inv${Date.now()}`, fromUserId: currentUser.id, toUserId: targetUser.id, status: 'pending', date: new Date().toISOString() };
    ...
    const handleSettleDebt = (debt: Debt) => { setDebtToPay(debt); };

    const handleConfirmPayment = (receiptUrl?: string) => {
        if (!debtToPay || !selectedGroupId) return;
        const newTransaction: Transaction = {
            id: `t${Date.now()}`, description: `Pagamento para ${debtToPay.to.name}`, amount: debtToPay.amount, payers: [{ userId: debtToPay.from.id, amount: debtToPay.amount }],
            groupId: selectedGroupId, date: new Date().toISOString(), category: 'transfer', participantIds: [debtToPay.to.id], receiptUrl: receiptUrl
        };
        ...
        if (currentUser) {
             ...
             if (settings && settings.debts) {
                 setCustomNotifications(prev => [...prev, {
                    id: `pay_${Date.now()}`, userId: debtToPay.to.id, type: 'debt', title: 'Pagamento Recebido', message: `${currentUser.name} registrou um pagamento de R$ ${debtToPay.amount.toFixed(2)} para você.`, date: new Date().toISOString(), read: false, actionLink: 'Dashboard'
                 }]);
             }
        }
    };
    ...
    // Render Logic
    if (isAuthLoading) {
        return <div className="flex h-screen bg-slate-900 items-center justify-center text-teal-500"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-teal-500"></div></div>;
    }
    ...
    const renderContent = () => {
        switch (activeView) {
            case 'Dashboard': return <Dashboard debts={debts} balances={memberBalances} currentUser={currentUser} onSettleDebt={handleSettleDebt} isPersonal={isPersonalGroup} totalSpent={groupTransactions.reduce((sum, t) => sum + t.amount, 0)} revenues={userRevenues} />;
            case 'Despesas': return <TransactionHistory transactions={groupTransactions} users={selectedGroup.members} onNewTransaction={() => setIsExpenseModalOpen(true)} onViewTransaction={setViewingTransaction} />;
            case 'Receitas': return <RevenuePage revenues={userRevenues} onAddRevenueClick={() => setIsRevenueModalOpen(true)} onToggleReceived={handleToggleRevenueReceived} onDeleteRevenue={handleDeleteRevenue} />;
            case 'Metas': return <GoalsPage goals={groupGoals} contributions={contributions} onOpenAddGoalModal={() => setIsAddGoalModalOpen(true)} onOpenAddContributionModal={handleOpenAddContributionModal} onViewGoalDetails={setViewingGoal} />;
            case 'Amigos': return <FriendsPage currentUser={currentUser} users={users} friends={friends} pendingInvitations={pendingInvitations} onSendInvitation={handleSendInvitation} onUpdateInvitation={handleUpdateInvitationStatus} />;
            case 'Relatórios': return <ReportsPage groupName={selectedGroup.name} transactions={groupTransactions} members={selectedGroup.members} />;
            case 'Assistente IA': return <AiAssistantPage group={selectedGroup} transactions={groupTransactions} goals={groupGoals} members={selectedGroup.members} currentUser={currentUser} />;
            default: return <Dashboard debts={debts} balances={memberBalances} currentUser={currentUser} onSettleDebt={handleSettleDebt} isPersonal={isPersonalGroup} totalSpent={groupTransactions.reduce((sum, t) => sum + t.amount, 0)} revenues={userRevenues} />;
        }
    };
    ...
    return (
        <div className="flex h-screen bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-gray-200 transition-colors duration-200">
            ...
            <Sidebar 
                groups={groups} selectedGroupId={selectedGroupId!} onSelectGroup={handleSelectGroup} onOpenAddGroupModal={() => setIsGroupModalOpen(true)}
                activeView={activeView} onSelectView={setActiveView} onOpenSettings={() => setIsUserSettingsModalOpen(true)} onLogout={handleLogout}
                isOpen={isMobileMenuOpen} onClose={() => setIsMobileMenuOpen(false)}
            />
            <main className="flex-1 flex flex-col overflow-hidden w-full relative">
                <Header 
                    group={selectedGroup} currentUser={currentUser} onOpenSettings={() => setIsGroupSettingsModalOpen(true)} showSettings={activeView === 'Dashboard' && !isPersonalGroup}
                    notifications={allNotifications} onMarkNotificationAsRead={markAsRead} onClearNotifications={clearAll} onNavigate={setActiveView} onOpenMobileMenu={() => setIsMobileMenuOpen(true)}
                />
                <div className="flex-1 overflow-y-auto p-4 md:p-8">
                    {activeView === 'Dashboard' ? (
                        <div className="space-y-8">
                             <Dashboard debts={debts} balances={memberBalances} currentUser={currentUser} onSettleDebt={handleSettleDebt} isPersonal={isPersonalGroup} totalSpent={groupTransactions.reduce((sum, t) => sum + t.amount, 0)} revenues={userRevenues} />
                            {!isPersonalGroup && debts.length > 0 && (
                                <div className="bg-white dark:bg-slate-800/50 p-6 rounded-lg shadow-sm border border-gray-200 dark:border-transparent">
                                     ...
                                     <div className="grid gap-4">
                                        {debts.map((debt, i) => debt.from.id === currentUser?.id && (
            ...
            {isExpenseModalOpen && <AddExpenseModal group={selectedGroup} onClose={() => setIsExpenseModalOpen(false)} onAddExpense={handleAddExpense} />}
            {isRevenueModalOpen && <AddRevenueModal onClose={() => setIsRevenueModalOpen(false)} onAddRevenue={handleAddRevenue} />}
            {isGroupModalOpen && <AddGroupModal availableMembers={[currentUser, ...friends]} currentUser={currentUser} onClose={() => setIsGroupModalOpen(false)} onAddGroup={handleAddGroup} />}
            {viewingTransaction && <ExpenseDetailModal transaction={viewingTransaction} group={selectedGroup} onClose={() => setViewingTransaction(null)} onUpdateTransaction={handleUpdateTransaction} onDeleteTransaction={handleDeleteTransaction} />}
            {isGroupSettingsModalOpen && <GroupSettingsModal group={selectedGroup} allUsers={users} friends={friends} currentUser={currentUser} onClose={() => setIsGroupSettingsModalOpen(false)} onUpdateMembers={handleUpdateGroupMembers} onDeleteGroup={handleDeleteGroup} />}
            {isAddGoalModalOpen && <AddGoalModal onClose={() => setIsAddGoalModalOpen(false)} onAddGoal={handleAddGoal} />}
            {isAddContributionModalOpen && selectedGoalForContribution && <AddContributionModal goal={selectedGoalForContribution} groupMembers={selectedGroup.members} onClose={() => { setIsAddContributionModalOpen(false); setSelectedGoalForContribution(null); }} onAddContribution={handleAddContribution} />}
            {viewingGoal && <GoalDetailModal goal={viewingGoal} contributions={contributions.filter(c => c.goalId === viewingGoal.id)} groupMembers={selectedGroup.members} onClose={() => setViewingGoal(null)} onOpenAddContributionModal={() => setSelectedGoalForContribution(viewingGoal)} />}
            {isUserSettingsModalOpen && <UserSettingsModal user={currentUser} onClose={() => setIsUserSettingsModalOpen(false)} onUpdateUser={handleUpdateUser} currentTheme={theme} onToggleTheme={toggleTheme} onDeleteAccount={handleDeleteAccount} />}
            {debtToPay && <PixPaymentModal debt={debtToPay} onClose={() => setDebtToPay(null)} onConfirmPayment={handleConfirmPayment} />}
        </div>
    );
};
